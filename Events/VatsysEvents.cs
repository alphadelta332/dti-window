...
namespace DTIWindow.Events
{
    public class VatsysEvents : BaseForm
    {
        ...
        public void InitialiseSliderScroll()
        {
            try
            {
                // Get the DisplaySettingsWindow type
                Type? displaySettingsWindowType = typeof(MMI).Assembly.GetType("vatsys.DisplaySettingsWindow");
                if (displaySettingsWindowType == null)
                {
                    Debug.WriteLine("DisplaySettingsWindow type not found.");
                    return;
                }

                // Get the currently open DisplaySettingsWindow instance
                var displaySettingsWindow = Application.OpenForms
                    .OfType<Form>()
                    .FirstOrDefault(f => f.GetType() == displaySettingsWindowType);

                if (displaySettingsWindow == null)
                {
                    Debug.WriteLine("DisplaySettingsWindow instance not found.");
                    return;
                }

                // Access the jurisSlider field using reflection
                var jurisSliderField = displaySettingsWindowType.GetField("jurisSlider", BindingFlags.NonPublic | BindingFlags.Instance);
                if (jurisSliderField == null)
                {
                    Debug.WriteLine("jurisSlider field not found.");
                    return;
                }

                // Get the jurisSlider instance
                var jurisSlider = jurisSliderField.GetValue(displaySettingsWindow);
                if (jurisSlider == null)
                {
                    Debug.WriteLine("jurisSlider instance not found.");
                    return;
                }

                // Get the Scroll event of the jurisSlider
                var scrollEvent = jurisSlider.GetType().GetEvent("Scroll");
                if (scrollEvent == null)
                {
                    Debug.WriteLine("Scroll event not found on jurisSlider.");
                    return;
                }

                // Create a delegate for the OnJurisSliderScroll method
                MethodInfo onJurisSliderScrollMethod = typeof(VatsysEvents).GetMethod(nameof(OnJurisSliderScroll), BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (onJurisSliderScrollMethod == null)
                {
                    Debug.WriteLine("OnJurisSliderScroll method not found.");
                    return;
                }

                Delegate scrollEventHandler = Delegate.CreateDelegate(scrollEvent.EventHandlerType, this, onJurisSliderScrollMethod);

                // Add the event handler to the Scroll event
                scrollEvent.AddEventHandler(jurisSlider, scrollEventHandler);

                Debug.WriteLine("Successfully hooked into jurisSlider Scroll event.");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Exception while hooking into jurisSlider Scroll event: {ex.Message}\n{ex.StackTrace}");
            }
        }

        private void OnJurisSliderScroll(object sender, EventArgs e)
        {
            try
            {
                Debug.WriteLine("jurisSlider Scroll event triggered.");

                // Perform your custom logic here
                // For example, refresh the UI
                var windowInstance = Application.OpenForms
                    .OfType<Window>()
                    .FirstOrDefault();
                windowInstance?.PopulateAircraftDisplay();
            ...
            {
                Debug.WriteLine($"Exception in OnJurisSliderScroll: {ex.Message}\n{ex.StackTrace}");
            }
        }
    }
}